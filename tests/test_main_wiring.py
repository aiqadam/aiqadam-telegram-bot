"""Tests for main.py's dispatcher wiring (FR-BOT-002 PR 1/6).
...
BOT_COMMANDS content (in particular: /event is deliberately excluded, per
main.py's own comment) and that all five command routers are registered.
"""
...
def test_bot_commands_excludes_event_since_it_requires_an_argument() -> None:
...
def test_build_dispatcher_registers_all_expected_routers(tmp_path) -> None:
    ...
    assert {"start", "help", "events", "event_detail", "fallback", "errors"}.issubset(router_names)
